...
int main(void)
{
    ...
    while (playAgain) {
        ...
        if (won)
        {
           cout<<"********HURRAYYYYYY YOU WON :)!********"<<endl;
           cout << "The correct sequence was " << right[0] << " "
...
void prompt(char guess[], int n)
{
     ...
     string validcolorset ="BGOPRY";

     for (int i = 0; i < 4; i++)
     {
         std::size_t isvalidset = validcolorset.find(guess[i]);
         if (isvalidset != string::npos)
         {
            if (guess[i] > 91) // Converting lower case level to upper case
                guess[i] = guess[i] - 32;
         }
         else
         {
          cout <<"********INVALID CHARACTER SET********"<<endl;
          cout <<"$$$$$Please enter Blue, Green, Orange, Purple, Red and Yellow only$$$$$"<<endl;
          prompt(guess,n);
         }
     }
